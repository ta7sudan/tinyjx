...
let jsonpId = Date.now(), cacheRand = Date.now() + 5, globalSerialize = null, globalDeserialize = null;
function createXhr() {
...
const defaultSerialize = ({ data, method, contentType = MIME.json, url, cache }) => {
    if (!cache) {
...
const defaultDeserialize = ({ data, contentType, acceptType }) => {
    let rst = null;
...
function jsonp(opts) {
    let { url, 
    /* tslint:disable */
    cache = false, crossorigin, callbackName, success, beforeSend, complete, error
    /* tslint:enable */
     } = opts;
    ...
    const hasSuccessCb = isFn(success), hasErrorCb = isFn(error), hasCompleteCb = isFn(complete), 
    ...
    cbName = hasCustomCbName ? callbackName : `jsonp${++jsonpId}`, script = document.createElement('script');
...
function ajax(opts) {
    let { url = lc.href, method = 'GET', contentType: reqCtype, dataType: acceptType = 'json', 
    /* tslint:disable */
    data: reqRawData, beforeSend, complete, error, headers, mimeType, responseType = '', username, password, success, timeout = 0, ontimeout, events, uploadEvents, withCredentials = false, cache = true, serialize, deserialize
    /* tslint:enable */
     } = opts;
    ...
    const slz = isFn(serialize)
        ...
        : isFn(globalSerialize)
            ? globalSerialize
            : defaultSerialize, dslz = isFn(deserialize)
        ? deserialize
        : isFn(globalDeserialize)
            ? globalDeserialize
            : defaultDeserialize, maybeProtocol = /^([\w-]+:)\/\//.exec(url), hrefProtocol = /^(https?):\/\//.exec(lc.href), protocol = maybeProtocol ? maybeProtocol[1] : hrefProtocol ? hrefProtocol[1] : null, xhr = xhrFactory(), hasCompleteCb = isFn(complete), hasErrorCb = isFn(error), hasSuccessCb = isFn(success);
    let reqData, errCalled = false, completeCalled = false;
...
function config({ pool = false, serialize, deserialize } = {}) {
    if (pool) {
        xhrPool.length = 0;
...
function get(url, opts) {
    return ajax(Object.assign({}, opts, { url, method: 'GET' }));
}
function head(url, opts) {
    return ajax(Object.assign({}, opts, { url, method: 'HEAD' }));
}
function post(url, data, opts) {
    return ajax(Object.assign({}, opts, { url,
        data, method: 'POST' }));
}
